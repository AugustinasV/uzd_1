...
int linecount = 0;
int errorcount = 0;
const int columncount = 6; /// file column count
...
int main(){
    ...
    for (int i=0;i < grupe.size(); i++) {
        if (grupe[i].galutinis_paz < 5) {
            neig.push_back(grupe[i]);
        }
        else {
            teig.push_back(grupe[i]);
        }
    }
    cout << std::to_string(size)+".txt sort: "  << c.elapsed() << "ms"<< endl;
    totaltime +=c.elapsed();

    timer d;
    ofstream file1(std::to_string(size)+"<5.txt");
    for (int q=0; q<neig.size(); q++) {
        file1 << neig[q].vardas << " " << neig[q].pavarde << " ";
        for (int i=0; i < neig[q].nd_paz.size(); i++) {
            file1 << neig[q].nd_paz[i] << " ";
        }
        file1 << neig[q].egz_paz << "\n";
    }
    file1.close();
    ofstream file2(std::to_string(size)+">=5.txt");
    for (int q=0; q<teig.size(); q++) {
        file2 << teig[q].vardas << " " << teig[q].pavarde << " ";
        for (int i=0; i < teig[q].nd_paz.size(); i++) {
            file2 << teig[q].nd_paz[i] << " ";
        }
        file2 << teig[q].egz_paz << "\n";
    }
    
    file2.close();
    cout << std::to_string(size)+".txt write: "  << d.elapsed() << "ms"<< endl;
    totaltime +=d.elapsed();
    cout << "=================================" << endl;
